...
# Archivos y librerías de la sesión
...
La matriz tiene 500 documentos y 16564 términos. 
Pero, **¿por qué son tantos términos?** 
Lo que estamos haciendo es representar los datos como una *bolsa de palabras* (Bag of Words o BOW), donde cada fila representa un documento, y las columnas, los términos o palabras de cada documento. 
Por ejemplo, supongamos que tenemos 2 documentos:

1. eres el iman y yo soy el metal
2. yo si se quien eres

Al conjunto de documentos se le llama **corpus**.
Si juntamos todas las palabras, tenemos 11 términos (eres, el, iman, y, yo, soy, el, metal, si, se, quien). 
A estos términos del corpus se le llama **Vocabulario** puesto que representan todas las palabras de todos los documentos. 
Por lo tanto, la matriz quedaría de la forma:
...
La matriz M, está pre-procesada de acuerdo a la última estrategia. 

- Por otra parte, un término puede ser una *palabra*, o un $n$-grama (secuencias de $n$ palabras que aparecen juntas); por ejemplo, un bigrama (o $2$-grama) corresponde a un par de palabras. En este caso cada término corresponde a una palabra.

- Otro preprocesamiento hecho sobre los datos corresponde a eliminar *stopwords* o palabras muy frecuentes en el lenguaje (en el caso del español: las preposiciones, pronombres, artículos, etc.). Es posible determinar metodológicamente las stopwords en un corpus dado de acuerdo al dominio del problema, ya que pueden depender del idioma, o del vocabulario usado.

- Otro preprocesamiento corresponde a *lematizar* (*stemming*) las palabras para mantener una raíz única. Por ejemplo: 'presidenta' y 'presidente' corresponden al mismo concepto, pero son palabras distintas, por lo que al lematizar, nos quedará como resultado 'president'. Por supuesto, habrá casos en los cuales no querremos lematizar, por ejemplo, si nos importa el género de las palabras, como en el caso anterior.

- Finalmente, espacios en blanco extra, números y y símbolos fueron removidos.

¿Por qué hacemos todo este pre-procesamiento? El objetivo es reducir la cantidad de dimensiones lo más posible. Aun con el pre-procesamiento realizado, tenemos más de 16 mil dimensiones para sólo 500 instancias o documentos. Esto hace que el problema de clustering sea impráctico de realizar manualmente.
