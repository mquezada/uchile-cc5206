...
Acá se puede notar que un valor óptimo es 2 (mirar donde se forma un "codo"). Si eligieramos 3,4 ó 5, veríamos más particiones, pero quizás no son tan claras esas particiones. 
...
Podemos ver si hay grupos entre pares de variables usando `pairs`. Acá podemos notar que hay una separación en los datos. Pueden ser 2 o más. 
```{r}
pairs(x)
```
...
En este caso, se puede apreciar el codo el *k = 2* (o quizás *k = 6*, habría que probar). 
...
Otra forma de graficar (más clara), con *clusplot()*: 
...
data("iris")
x <- as.matrix(iris[, 1:4])   # tomamos las columnas 1 a la 4 solamente, sin la clase
